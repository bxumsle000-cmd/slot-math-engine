...
def calculate_rtp(  # 計算多線理論 RTP（向量化，毫秒級）
    ...
    算法分三個概念區塊：
        ...
        【第一張量】賠付倍率張量 combo_payout_tensor，shape (7,7,7,7,7)
            遍歷所有 7^5 = 16,807 種符號組合，查賠付表得倍率，填入張量。
            不中獎的組合填 0。此張量與付線無關，只算一次。

        【第二張量】機率張量 P，shape (7,7,7,7,7)，每條付線各算一次
            因各軸獨立，P[i0,i1,i2,i3,i4] = 各軸在付線指定行出現對應符號的機率之積。
    ...
    # ══════════════════════════════════════════════════════════════════════════
    # 前置 1/2：收集所有出現在捲軸帶上的符號，建立索引映射
    # ══════════════════════════════════════════════════════════════════════════
    #
    # 為什麼需要「整數索引」？
    #   numpy 陣列只能用整數當索引，不能用字串。
    #   所以先把所有符號名稱收集起來、排序後，給每個符號一個固定的整數編號。
    #
    #   例如，符號共 7 種，排序後：
    #       0: BAR
    #       1: Blank
    ...
    # ── 前置 2/2：計算每條捲軸在每行的符號出現機率 ──────────────────────────────
    #
    # 老虎機每次旋轉時，每條捲軸會「隨機停在某個位置」，然後露出連續 3 個符號（上中下行）。
    # 因為每個停格的機率相等，所以「某符號出現在某行的機率」= 該符號出現在那行的停格數 / 總停格數。
    #
    ...
    #   ...
    #
    # 這裡用「讓捲軸跑過所有停格，累計每行各符號的出現次數」來計算機率。
    # 每次停格貢獻 1/total_stops 的機率。
    sym_prob_all_reels: list[np.ndarray] = []
    for reel in reel_strips:
        # 初始化一張全為 0 的表，shape: (符號種類數, 行數=3)
        sym_prob_matrix = np.zeros((n_sym_types, 3))
        for s in range(reel.total_stops):       # 跑過這條捲軸所有停格
            for r in range(3):                  # 每個停格露出上(0)、中(1)、下(2) 三行
                # reel.window(s)[r] = 停格 s 時第 r 行顯示的符號名稱
                # sym_idx[...] 把符號名稱轉成整數索引，才能更新矩陣對應格子
                # += 1/total_stops：每個停格的機率等於 1/總停格數
                sym_prob_matrix[sym_idx[reel.window(s)[r]], r] += 1.0 / reel.total_stops
        sym_prob_all_reels.append(sym_prob_matrix)

    # ══════════════════════════════════════════════════════════════════════════
    # 【第一張量】賠付倍率張量 combo_payout_tensor
    #   遍歷所有 7^5 = 16,807 種符號組合，查賠付表，填入 shape (7,7,7,7,7) 的張量。
    #   不中獎填 0。此張量與付線無關，只需建立一次。
    # ══════════════════════════════════════════════════════════════════════════
    #
    # 第一張量 1/2：遍歷所有符號組合，建立「符號名稱 → 賠付倍率」的 dict
    #
    # 把 7 種符號排列組合成所有可能的 5 軸組合（7^5 = 16,807 種），
    # 每種組合丟進 _evaluate_payline() 查賠付，只保留有獎（倍率 > 0）的組合。
    #
    # 為什麼先建 dict，不直接建 numpy 陣列？
    #   因為 _evaluate_payline() 接受的是「符號名稱 tuple」，例如：
    #       ('Seven', 'Seven', 'Seven', 'Seven', 'Seven')
    #   還不能直接用整數索引。第一張量 2/2 才會轉換格式。
    #
    ...
    #
    # 為什麼要轉成 numpy 張量？
    #   【相乘求和】需要把「機率張量 P」和「賠付倍率張量」逐元素相乘，
    #   這個操作只有兩個 shape 相同的 numpy 陣列才能做，dict 無法直接參與。
    ...
    for pl in paylines:
        # 從完整的 (n_sym_types, 3) 機率矩陣，切出「此付線指定行」的機率向量
        # pl[k] 是付線在捲軸 k 指定的行號（0=上行, 1=中行, 2=下行）
        # [:, pl[k]] 取出整欄，得到 shape (n_sym_types,) 的向量，代表此行每種符號的出現機率
        #
        # reel_prob_vecs 是一個 list，長度 = 捲軸數（5 條），每個元素是 shape (n_sym_types,) 的向量
    ...
    # 各付線 RTP 取平均
    # 注意：押滿所有付線時，「總賠付 / 總押注」的線數會互相抵消，
    # 所以整體 RTP = 任一付線的 RTP = 各付線 RTP 的平均值。
    rtp_per_line = sum(payline_rtp) / len(paylines)

    # ══════════════════════════════════════════════════════════════════════════
    # 中獎明細（副產品）：以付線 0 為代表，列出每個中獎組合的機率與 RTP 貢獻
    # ══════════════════════════════════════════════════════════════════════════
    #
    # 為什麼只用付線 0 代表所有付線？
    ...
    #
    # ref_reel_prob_vecs 和步驟 5 的 reel_prob_vecs 結構完全一樣，
