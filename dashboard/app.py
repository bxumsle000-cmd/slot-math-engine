...
with st.sidebar:
    ...
    with st.expander("Free Spin 設定", expanded=True):
        ...
        # 發散防護：堆疊式 retrigger 下，N × 每局觸發機率 ≥ 1 時 FS 期望局數發散（RTP→∞）
        fs_diverges = free_spin_count * trigger_prob >= 1  # True = 目前參數使 FS 永不結束（含 FS 分析須暫停）
        _fs_diverge_msg = (  # 各 FS 分頁發散時共用的提示文字
            "含 Free Spin 的分析已暫停：目前參數使 FS 期望局數發散（RTP→∞）。"
            "請降低 Scatter 格數或 Free Spin 局數 N。"
        )
        if fs_diverges:
            st.warning(
                f"⚠ N × 觸發機率 = {free_spin_count} × {trigger_prob:.4f} = "
                f"{free_spin_count * trigger_prob:.2f} ≥ 1，{_fs_diverge_msg}"
            )
...
def cached_ml_freespin_rtp(n_fs: int, m: float, reel_cfg: dict[str, int]):  # 快取多線含 FS 的馬可夫理論 RTP（trigger/retrigger 由捲軸帶自動計算）
    ...
    """
    cfg_fs = FreespinConfig(trigger_prob=0.0, free_spin_count=n_fs, retrigger_prob=0.0, win_multiplier=m)  # trigger/retrigger 皆衍生，0.0 為佔位
    strip = _build_strip(reel_cfg)
...
def sensitivity_trigger_ml(n_fs: int, m: float, reel_cfg: dict[str, int]) -> tuple[list, list, list]:  # 掃描多線 Scatter 格數敏感度（sweep scatter 1~8）
    ...
    Returns:
        ...
        Scatter 過密致 N·r≥1（FS 期望局數發散）的點，RTP 以 NaN 表示，曲線在此斷開
    """
    ...
    for sc in scatter_counts:
        ...
        probs.append(p_trig)
        cfg = FreespinConfig(trigger_prob=0.0, free_spin_count=n_fs, retrigger_prob=0.0, win_multiplier=m)  # trigger/retrigger 皆衍生，0.0 為佔位
        try:
            rtps.append(calculate_freespin_rtp(cfg, reel_strips=strips).total_rtp * 100)
        except ValueError:
            # Scatter 過密使 N·r≥1：堆疊式 retrigger 讓 FS 期望局數發散、RTP→∞，
            # 標 NaN 使曲線在此斷開（此區為不合理機台設計，現實不會採用）
...
def cached_player_journeys(  # 快取玩家旅程模擬（按所有參數 key 快取）
    ...
    _v: int = 6,  # 快取版本號，結構變動時遞增以強制 cache miss
) -> PlayerJourneyResult:
    """
    快取玩家旅程模擬，key = 所有參數組合。
    ...
    Args:
        ...
        stop_win: 停利線
        max_spins: 最大局數
        use_fs: 是否啟用 Free Spin
        n_fs: FS 局數（use_fs=False 時忽略）
        m: 贏分倍率（use_fs=False 時忽略）
        s: 隨機種子
    ...
    """
    fs_cfg = (
        # trigger/retrigger 皆為佔位 0.0：simulate_player_journeys 以真實 Scatter 結果判定觸發與續場，
        # 不讀 config 的 trigger_prob/retrigger_prob（只用 free_spin_count 與 win_multiplier）
        FreespinConfig(trigger_prob=0.0, free_spin_count=n_fs, retrigger_prob=0.0, win_multiplier=m)
        if use_fs else None
    )
    strip = _build_strip(reel_cfg)
...
with tab4:
    ...
    if fs_diverges:
        st.warning(_fs_diverge_msg)
        st.stop()  # 發散時跳過玩家旅程模擬（含 FS 會觸發 core 的 ValueError）
    ...
    # 理論對照：每次觸發的期望續場次數 = 每局續場機率 r × 每次觸發平均 FS 局數 N/(1−N·r)
    # （不可直接拿 retrigger_prob 對照 _retrig_rate：前者是「每局機率」、後者是「每次觸發的次數」，量綱不同會差約 N/(1−N·r) 倍）
    _retrig_per_trigger = (
        retrigger_prob * free_spin_count / (1 - free_spin_count * retrigger_prob)
        if free_spin_count * retrigger_prob < 1 else float("inf")
    )
    ...
    col_fs3.metric(
        "續場率（每次觸發後）",
        f"{_retrig_rate:.4f}",
        f"理論 {_retrig_per_trigger:.4f}（每次觸發期望續場次數）",
    )
    col_fs4.metric(
        "平均續場次數（每位玩家）",
