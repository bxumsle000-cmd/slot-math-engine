...
class PlayerJourneyResult:
    ...
    fs_trigger_counts: list[int]           # 各玩家觸發 Free Spin 的次數（未觸發者為 0）
    fs_retrigger_counts: list[int]         # 各玩家發生續場（retrigger）的次數（未續場者為 0）
...
def simulate_player_journeys(  # 模擬多位玩家的完整遊戲旅程
    ...
    *,
    ...
    押注模型：每局押滿所有付線，每線各 1 單位，每局成本 = num_lines 。
    中獎收益 = 本局 total_multiplier；淨損益 = total_multiplier - num_lines。

    以「每局真實 Scatter 結果」判定是否觸發 Free Spin
    ...
    Args:
        ...
        fs_config: Free Spin 參數設定（必填）；提供 free_spin_count 與 win_multiplier
    ...
    # 理論 RTP 只需計算一次，作為對照參考（含 FS 的馬可夫理論值）
    ...
    for _ in range(num_players):
        ...
        for spin_num in range(max_spins):
            ...
            # ── 一局的收益計算（基礎局 + 觸發後的所有 FS 局）────────────────
            # 先執行基礎局，再以真實 Scatter 結果判斷是否觸發 FS
