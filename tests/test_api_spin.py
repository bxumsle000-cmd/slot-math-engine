...
def client_with_player():  # 建立 TestClient 並注入餘額 1000、free_spins_remaining=0 的測試玩家
    ...
    player = Player(username="tester", google_sub="test-sub-tester", email="tester@example.com", balance=1000.0, free_spins_remaining=0)  # 測試玩家（Google 身分），無剩餘 FS
    db.add(player)
    db.commit()
...
def client_with_fs_player():  # 建立 TestClient 並注入剩餘 5 局 FS 的測試玩家
    ...
    player = Player(
        username="fs_tester",
        google_sub="test-sub-fs-tester",   # Google 帳號唯一識別碼（測試用假值）
        email="fs_tester@example.com",      # Google 帳號 email（測試用假值）
        balance=1000.0,
